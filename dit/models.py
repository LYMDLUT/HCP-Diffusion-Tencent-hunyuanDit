...
class HunYuanDiT(ModelMixin, ConfigMixin):
    ...
    def from_pretrained(cls, model_path=None, pretrained=True):
        ...
        model = HunYuanDiT(input_size=latent_size,
                            learn_sigma=dit_args.learn_sigma,
                            text_states_dim=dit_args.text_states_dim,
                            text_states_dim_t5=dit_args.text_states_dim_t5,
                            text_len=dit_args.text_len,
        ...
        for layer, block in enumerate(self.blocks):
            if layer > self.depth // 2:
                ...
                if self.gradient_checkpointing:
                    x = checkpoint(block, x, c, text_states, freqs_cis_img, skip)   # (N, L, D)
                else:
                    x = block(x, c, text_states, freqs_cis_img, skip)   # (N, L, D)
            else:
                if self.gradient_checkpointing:
                    x = checkpoint(block, x, c, text_states, freqs_cis_img)         # (N, L, D)
                else:
